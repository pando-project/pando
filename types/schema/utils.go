...
func NewAdvertisement(
	...
	return newAdvertisement(signKey, previousID, entries, provider)
...
func NewAdvertisementWithLink(
	...
	entries ipld.Link,
	provider string,
) (Advertisement, Link_Advertisement, error) {

	// Create advertisement
	adv, err := newAdvertisement(signKey, previousID, entries, provider)
	if err != nil {
		return nil, nil, err
	}
	// Generate link
