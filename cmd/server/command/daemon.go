...
var logger = logging.Logger("pando")
...
func DaemonCmd() *cobra.Command {
	...
	return &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			logger.Debugf("metastore connected: %s", Opt.MetaStore.ConnectionURI)
			...
			// todo: test logger for dealbot integration
			go func() {
				peerID, err := peer.Decode("12D3KooWNnK4gnNKmh6JUzRb34RqNcBahN5B8v18DsMxQ8mCqw81")
				if err != nil {
					logger.Errorf("wrong dealbot peerid: %s", err.Error())
					return
				}
				...
				for range t.C {
					...
					if info == nil {
						logger.Debugf("dealbot not register")
						continue
					}
					maddrs := p2pHost.Peerstore().Addrs(info[0].AddrInfo.ID)
					for _, maddr := range maddrs {
						logger.Debugf("dealbot maddrs: %s", maddr.String())
						...
						if err != nil {
							logger.Debugf("sync from dealbot failed(maddr: %s), error: %v", maddr, err)
							continue
						}
						logger.Debugf("sync from dealbot success, cid: %s", syncedCid.String())
					}
				}
			}()
