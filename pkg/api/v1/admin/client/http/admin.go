...
import (
	...
	httpclient2 "pando/pkg/httpclient"
...
func New(baseURL string, options ...httpclient2.Option) (*Client, error) {
	u, c, err := httpclient2.New(baseURL, "", adminPort, options...)
	if err != nil {
		return nil, err
	}
	baseURL = u.String()
...
func (c *Client) Register(ctx context.Context, providerID peer.ID, privateKey p2pcrypto.PrivKey, addrs []string, miner string) error {
	...
	if resp.StatusCode != http.StatusOK {
		return httpclient2.ReadError(resp.StatusCode, body)
	}
	return nil
}
